...
const Group: React.FC<GroupProps> = ({
  ...
  return (
    <div
      className={`p-4 border ${borderColors[color]} rounded-lg bg-gradient-to-br ${bgColors[color]} ${className}`}
    >
      {title && (
        <div className="mb-3 flex items-center">
          <h3 className="text-sm font-medium text-white">{title}</h3>
          <div
            className={`ml-2 w-full h-px bg-gradient-to-r ${borderColors[color]}`}
          ></div>
        </div>
      )}
...
export default function InfrastructureDiagramPage() {
  ...
  const awsDataObj = (awsData as { status: string; region: string }) || {
  ...
  const terraformStatusObj = (terraformStatus as {
    status: string;
    provider: string;
    region: string;
  ...
  const steps = [
    {
      ...
      description:
        "Store the application code in Git. Write a Dockerfile for containerization.",
    },
    {
      id: 2,
      title: "CI Pipeline",
      description:
        "Upon code pushes, automatically run tests. If tests pass, build a Docker image and push it to a registry.",
    },
    {
      id: 3,
      title: "Infrastructure as Code",
      description:
        "Provision your environment using Terraform. Create compute instances and networking resources.",
    },
    {
      id: 4,
      title: "Deployment",
      description:
        "Deploy the container to the cloud environment. Confirm the service is accessible.",
    },
  ];
            ...
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="mt-1 max-w-2xl text-sm text-gray-200">
                  Visual representation of your DevOps pipeline and cloud
                  deployment
                </p>
              </div>
            </div>
            ...
            <div className="relative min-h-[700px] bg-gradient-to-br from-gray-900 to-gray-950 rounded-lg p-8 overflow-auto border border-gray-800">
                ...
                <div className="flex justify-between items-center mb-8">
                  ...
                  <div className="flex flex-col items-center mx-8 max-w-xs">
                    ...
                    <div className="mt-4 space-y-3 bg-gray-800/30 p-3 rounded-md border border-purple-500/20 w-full">
                      <div
                        className={`flex items-center space-x-2 transition-opacity duration-300 ${highlightStep(2) ? "opacity-100" : "opacity-30"}`}
                      >
              ...
              {(highlightStep(3) || highlightStep(4)) && (
                <div className="relative mt-3">
                  ...
                  <div
                    className={`flex justify-center my-5 transition-opacity duration-300 ${highlightStep(3) ? "opacity-100" : "opacity-30"}`}
                  >
